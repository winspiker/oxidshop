    ...
    [{oxscript add="var autoc = $( '#searchParam' ).autocomplete({
			...
			source: function( request, response ) {
				$.ajax({
					url: '"|cat:$oViewConf->getBaseDir()|cat:"index.php?cl=exonnsearch&type=JSON',
					dataType: 'jsonp',
					data: {
						maxRows: 12,
    ...
    <form class="form search" id="exonnsearch" action="[{ $oViewConf->getSelfActionLink() }]" method="get" name="search">
        <div class="searchBox">
            ...
            <input type="hidden" name="cl" value="exonnsearch">

            [{block name="dd_widget_header_search_form_inner"}]
            <div class="input-group searchBox">
                [{block name="header_search_field"}]
                <div class="form-control">
                    <input type="text" 	id="searchParam" name="searchparam" value="[{$oView->getSearchParamForHtml()}]" placeholder="[{oxmultilang ident="FIND_PRODUCTS"}]">
                </div>
                [{/block}]
