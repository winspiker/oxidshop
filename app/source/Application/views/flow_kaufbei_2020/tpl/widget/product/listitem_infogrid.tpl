    ...
    <form name="tobasket[{$testid}]" id="tobasket[{$testid}]" [{if $blShowToBasket}]action="[{$oViewConf->getSelfActionLink()}]" method="post"[{else}]action="[{$_productLink}]" method="get"[{/if}]>
        ...
        <div class="picture text-center">
            ...
            <a href="[{$_productLink}]" title="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
                ...
                <img src="[{$oViewConf->getImageUrl('spinner.gif')}]" data-src="[{$product->getThumbnailUrl()}]" alt="[{ $product->getTabslImageTag($product->oxarticles__tabsl_imagetag1->value) }]" title="[{ $product->getTabslImageTag($product->oxarticles__tabsl_imagetag1->value) }]" class="img-responsive">
            </a>
        </div>
        [{/block}]
