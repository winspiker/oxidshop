       ...
       [{assign var="cross" value='_'|explode:$testid}]
        ...
        <div class="picture text-center">
            ...
            <a href="[{$_productLink}]" title="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
                ...
                <svg class="image-rect" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 800">
                    <rect class="cls-1" width="364" height="364"/>
                </svg>

                <img src="[{$oViewConf->getImageUrl('spinner.gif')}]" data-src="[{$product->getPictureUrl(1)}]" alt="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]" class="img-responsive">
            </a>
        </div>
        [{/block}]
        ...
        <div class="listDetails">
            [{block name="widget_product_listitem_infogrid_titlebox"}]
                    ...
                    <a class="noStockCrossPopUptitle" id="[{$testid}]" href="[{$_productLink}]" title="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
                    ...
                    [{block name="widget_product_listitem_grid_price"}]
                        [{oxhasrights ident="SHOWARTICLEPRICE"}]
                            ...
                            [{if $tprice && $tprice->getBruttoPrice() > $price->getBruttoPrice()}]
                                <span class="oldPrice text-muted" style="display: block;">
                                    <!-- -->
                            ...
                            [{block name="widget_product_listitem_line_price_value"}]
                                <span class="prod_price_visiable" id="productPrice_[{$testid}]" class="lead text-nowrap">
                                    <span class="finalSum">
                                        [{if $product->isRangePrice()}]
                                            [{oxmultilang ident="PRICE_FROM"}]
                                            [{if !$product->isParentNotBuyable()}]
                                                [{assign var="finalSum"    value=$product->getFMinPrice() }]
                                                [{$product->getFMinPrice()}]
                                            [{else}]
                                                [{assign var="finalSum"    value=$product->getFVarMinPrice() }]
                                                [{$product->getFVarMinPrice()}]
                                            [{/if}]
                                        [{else}]
                                            [{if !$product->isParentNotBuyable()}]
                                                [{$product->getFPrice()}]
                                                [{assign var="finalSum"    value=$product->getFPrice() }]
                                            [{else}]
                                                [{$product->getFVarMinPrice()}]
                                                [{assign var="finalSum"    value=$product->getFVarMinPrice() }]
                                            [{/if}]
                        ...
                        [{/if}]
                            ...
                            [{if $oxcmp_user}]
                                ...
                                [{else}]
                                    ...
                                    <a class="btn actions--add-wishlist hasTooltip" href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account" params="anid=`$oDetailsProduct->oxarticles__oxnid->value`"|cat:"&amp;sourcecl="|cat:$oViewConf->getTopActiveClassName()|cat:$oViewConf->getNavUrlParams()}]" data-placement="bottom" title="[{oxmultilang ident="ADD_TO_WISH_LIST"}]"><i class="fa fa-heart" aria-hidden="true"></i></a>
                            [{/if}]
