...
<table id="basket_table" class="table table-striped basketitems[{if $oViewConf->getActiveClassName() == 'order'}] orderBasketItems[{/if}]">
    <colgroup>
    ...
    [{* basket items *}]
    <tbody class="basketTotalForm--container">
        ...
        [{foreach key=basketindex from=$oxcmp_basket->getContents() item=basketitem name=basketContents}]
            [{block name="checkout_basketcontents_basketitem"}]
                ...
                <tr id="table_cartItem_[{$smarty.foreach.basketContents.iteration}]" class="basketTotalForm--item">

                    [{block name="checkout_basketcontents_basketitem_titlenumber"}]
                        [{* product title & number *}]
                        <td class="basket-content">
                            [{block name="checkout_basketcontents_basketitem_image"}]
                                ...
                                <div class="pull-left basketImage">
                                    [{if $editable}]<a href="[{$basketitem->getLink()}]">[{/if}]
                                        <img src="[{$basketitem->getIconUrl()}]" data-src="[{$basketitem->getIconUrl()}]" alt="[{$basketitem->getTitle()|strip_tags}]">
                                        [{if $editable}]</a>[{/if}]
                            ...
                            <div class="basketItemDesc">
                                ...
                                [{block name="dd_checkout_inc_basketcontents_table_item_desc"}]
                                    ...
                                    <div class="smallFont">
                                        ...
                                        <script>
                                            fbq_items.push(String([{$basketproduct->oxarticles__oxartnum->value}]));
                                        </script>
                                    </div>
                                    <div class="smallFont">
                            ...
                            [{if $editable}]
                                ...
                                [{if !$basketitem->isBundle() || !$basketitem->isDiscountArticle()}]
                                    <div class="number-icon">  
                                         ...
                                         <input id="am_[{$smarty.foreach.basketContents.iteration}]" type="text" class="textbox" name="aproducts[[{$basketindex}]][am]" value="[{$basketitem->getAmount()}]" size="3" min="0">
