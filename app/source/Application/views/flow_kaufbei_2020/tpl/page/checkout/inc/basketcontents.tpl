...
[{if $oViewConf->getShowVouchers() && $oViewConf->getActiveClassName() == 'basket'}]
    [{block name="checkout_basket_vouchers"}]
        <div id="basketVoucher" class="pull-left">
            <form name="voucher" action="[{$oViewConf->getSelfActionLink()}]" method="post" class="js-oxValidate form-inline" role="form" novalidate>
        ...
        <table class="table table-bordered table-striped">
            ...
            [{block name="checkout_basketcontents_delcosts"}]
                ...
                [{if $deliveryCost && ($oxcmp_basket->getBasketUser() || $oViewConf->isFunctionalityEnabled('blCalculateDelCostIfNotLoggedIn') ) }]
                    ...
                    [{else}]
                    <tr>
                        <th>[{ oxmultilang ident="SHIPPING_COST" }]*:<div style="font-size: 12px;">* die Kosten werden nach Empfängeradresseingabe ermittelt</div></th>
            ...
            [{block name="checkout_basketcontents_grandtotal"}]
                <tr>
                    <th class="lead"><strong>[{oxmultilang ident="GRAND_TOTAL"}] ([{oxmultilang ident="INCLUDE_VAT"}]):</strong></th>
                    <td id="basketGrandTotal" class="lead"><strong>[{oxprice price=$oxcmp_basket->getPrice() currency=$currency}]</strong></td>
                    <script>
                        var fbq_price = '[{oxprice price=$oxcmp_basket->getPrice() currency=$currency}]'.split(' ')[0].replace(',', '.');
                    </script>
                </tr>
            [{/block}]
