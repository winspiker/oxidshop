    ...
    [{block name="checkout_basket_main"}]
        ...
        [{if !$oxcmp_basket->getProductsCount()}]
            [{block name="checkout_basket_emptyshippingcart"}]
                <div class="" id="empty-basket-warning">
                    <span>
                    <span class="kiconk-icon-cart"></span>
                        [{oxmultilang ident="BASKET_EMPTY"}]
                    </span>
                    <a href="[{$oViewConf->getHomeLink()}]" title="[{$oxcmp_shop->oxshops__oxtitleprefix->value}]" class="btn btn-default block-shadow"><i class="fa fa-caret-left"></i> [{oxmultilang ident="DD_BASKET_BACK_TO_SHOP"}]</a>
                </div>
            [{/block}]
        [{else}]
