...
<table class="orderarticles" width="100%" style="width:100%;">
    ...
    <tfoot>
        ...
        [{block name="email_html_order_cust_grandtotal"}]
            ...
            [{if $payment->oxpayments__installmentpayment->value}]
            <tr valign="top">
                <td style="padding: 5px; border-bottom: 2px solid #ccc;">
                    <p style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; margin: 0;">
                    <h3>Anzahlung insgesamt:</h3>
                    </p>
                </td>
                <td style="padding: 5px; border-bottom: 2px solid #ccc;" align="right">
                    <p style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; margin: 0;">
                    <h3>[{oxprice price=$basket->getFirstinstallmentTotal() currency=$currency}]</h3>
                    </p>
                </td>
            </tr>
            [{/if}]
...
[{block name="email_html_order_cust_paymentinfo_top"}]
    [{if $payment->oxuserpayments__oxpaymentsid->value != "oxempty"}]
        ...
        <p>
            <b>[{$payment->oxpayments__oxdesc->value}] </b>

            [{if $payment->oxpayments__installmentpayment->value}]
            Anzahlung insgesamt: [{oxprice price=$basket->getFirstinstallmentTotal() currency=$currency}]<br>
            [{/if}]
        </p>
        <br>
    [{/if}]
...
[{block name="email_html_order_cust_paymentinfo"}]
    [{if $payment->oxuserpayments__oxpaymentsid->value == "oxidpayadvance" || $payment->oxpayments__installmentpayment->value}]
        <h3 class="underline">[{oxmultilang ident="BANK_DETAILS"}]</h3>
        <p>
            [{oxmultilang ident="BANK"}] [{$shop->oxshops__oxbankname->value}]<br>
            [{oxmultilang ident="IBAN"}] [{$shop->oxshops__oxibannumber->value}]<br>
