...
class paypalpluscw_transaction_refund extends oxAdminDetails
{
	/**
		...
		if ($orderId != '-1' && isset( $orderId)) {
			...
			if (PayPalPlusCwHelper::isPaypalpluscwPaymentMethod($paymentId)) {
				...
				foreach ($transaction->getTransactionObject()->getRefunds() as $item) {
					if ($item->getRefundId() == oxRegistry::getConfig()->getRequestParameter('refund')) {
						$refund = $item;
						break;
					}
				}

				$this->_aViewData['refundId'] = $refund->getRefundId();

				$this->_aViewData['refundLabels'] = $refund->getRefundLabels();
				...
				$this->_aViewData['refundItems'] = $items;

				return "paypalpluscw_transaction_refund.tpl";
			}
		}
