    ...
    [{if $listType=='manufacturer' || $listType=='vendor'}]
        ...
        <div class="box">
            ...
            <div class="featured icon">
                ...
                <img src="[{$oViewConf->getImageUrl('spinner.gif')}]" data-src="[{$actCategory->getIconUrl()}]" alt="[{ $actCategory->getTabslImageTag("icon", $actCategory->oxcategories__tabsl_imagetag_icon->value) }]" title="[{ $actCategory->getTabslImageTag("icon", $actCategory->oxcategories__tabsl_imagetag_icon->value) }]">
...
[{capture append="oxidBlock_content"}]
    [{block name="page_list_listhead"}]
        [{if $actCategory->oxcategories__oxthumb->value && $actCategory->getThumbUrl()}]
            ...
            <img src="[{$oViewConf->getImageUrl('spinner.gif')}]" data-src="[{$actCategory->getThumbUrl()}]" alt="[{ $actCategory->getTabslImageTag("thumb", $actCategory->oxcategories__tabsl_imagetag_thumb->value) }]" title="[{ $actCategory->getTabslImageTag("thumb", $actCategory->oxcategories__tabsl_imagetag_thumb->value) }]" class="categoryPicture img-responsive img-thumbnail">
            [{* tabslImageTags 3.0.1 - END *}]
        [{/if}]
                                    ...
                                    <div class="panel-body">
                                        [{if $iconUrl}]
                                            <a href="[{$category->getLink()}]">
                                                ...
                                                <img src="[{$oViewConf->getImageUrl('spinner.gif')}]" data-src="[{$category->getIconUrl()}]" alt="[{ $category->getTabslImageTag("icon") }]" title="[{ $category->getTabslImageTag("icon") }]" class="img-responsive subcat-icon">
                                                [{* tabslImageTags 3.0.1 - END *}]
                                            </a>
                                        [{else}]
