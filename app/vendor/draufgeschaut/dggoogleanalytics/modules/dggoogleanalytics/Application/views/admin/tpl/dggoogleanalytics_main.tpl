...
<form name="transfer" id="transfer" action="[{ $oViewConf->getSelfLink() }]" method="post">
   ...
   <div class="groupExp">
        <div[{if $iStep == 2 }] class="exp"[{/if}]> 
            ...
            <dl>
                ...
                <dd>
                    <label for="dgGoogleAddWordAllwayActive"><font color="#838383" face="Arial Rounded MT Bold">ADWORDS</font> aktivieren in jeder Seite einblenden, nicht nur im Bestellprozess</label>
                </dd>
                <div class="spacer"></div>
            </dl>
             <dl>
   ...
   <div class="groupExp">
        <div[{if $iStep == "facebook" }] class="exp"[{/if}]> 
            ...
            <dl>
                ...
                <dd>
                    <label for="dgFaceBookRemaketingSmallActive">Facebook PIXEL ViewContent nur auf der Detailseite nutzen</label>
                </dd>
                <div class="spacer"></div>
            </dl>
            <dl>
                <dt></dt>
                <dd><button type="submit" value="" onclick="showPleaseWait();">speichern</button></dd>
                <div class="spacer"></div>
            </dl>
         </div>
    </div>
   ...
   <input type="hidden" name="updatenav" value="">
   <input type="hidden" name="editlanguage" value="[{ $editlanguage }]">
   <input type="hidden" name="iStep" value="adcell" /> 
   <div class="groupExp">
        <div[{if $iStep == "adcell" }] class="exp"[{/if}]> 
            <a href="#" onclick="_groupExp(this);return false;" class="rc"><b>Adcell Tracking Einstellungen</b></a>
            <dl>
                <dt>
                   <input type="hidden" name="confbools[dgAdCellActive]" value="false" />
                   <input id="dgAdCellActive" type="checkbox" name="confbools[dgAdCellActive]" value="true"  [{if ($confbools.dgAdCellActive)}]checked[{/if}]>
                </dt>
                <dd>
                    <label for="dgAdCellActive">Adcell Tracking aktivieren</label>
                </dd>
                <div class="spacer"></div>
            </dl>             
            <dl>                                                             
                <dt>
                   <input size="25" type="text" name="confstrs[dgAdCellAccountid]" value="[{ $confstrs.dgAdCellAccountid }]" class="dgInput" />
                </dt>
                <dd>
                     Adcell ACCOUNT ID 
                </dd>
            ...
            <dl>
                <dt>
                   <select name="confstrs[dgAdCellProductId]" size="1" class="dgInput">
                     <option value=""> - bitte w&auml;hlen - </option>
				     [{foreach from=$oView->getTable('oxarticles') key=field item=desc}]
                     [{assign var="ident" value=GENERAL_ARTICLE_$desc}]
                     [{assign var="ident" value=$ident|oxupper }]
                      <option value="oxarticles__[{ $desc|oxlower }]" [{ if $confstrs.dgAdCellProductId == "oxarticles__"|cat:$desc|oxlower }]SELECTED[{/if}]>[{ oxmultilang|oxtruncate:20:"..":true ident=$ident noerror=true alternative=$desc }]</option>
                     [{/foreach}]
                   </select>
                </dt>                
                <dd>
