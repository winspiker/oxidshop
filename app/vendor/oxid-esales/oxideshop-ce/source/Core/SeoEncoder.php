...
class SeoEncoder extends \OxidEsales\Eshop\Core\Base
{
    ...
    protected function _saveToDb($sType, $sObjectId, $sStdUrl, $sSeoUrl, $iLang, $iShopId = null, $blFixed = null, $sParams = null)
    {
        ...
        //exonn!
        //kaufbei will immer fixierte url haben
        $blFixedInsert = $blFixed;
        if ($this->getConfig()->getConfigParam('sSeoUrlFixed') /*ist in config datei definiert */ && $sType=="oxarticle" && $iLang==0) {
            $blFixedInsert = 1;
        }
        ...
        $sQ = "insert into oxseo
                ...
                values
                    ( {$sQtedObjectId}, {$sQtedIdent}, {$iQtedShopId}, {$iLang}, {$sQtedStdUrl}, {$sQtedSeoUrl}, {$sQtedType}, '$blFixedInsert', '0', {$sParams} )
