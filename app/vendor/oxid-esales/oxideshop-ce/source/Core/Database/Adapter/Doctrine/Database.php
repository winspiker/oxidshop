...
class Database implements DatabaseInterface
{
    ...
    public function getOne($query, $parameters = [])
    {
        ...
        if ($this->doesStatementProduceOutput($query)) {
            try {
                $res = $this->getConnection()->fetchColumn($query, $parameters);
                global $scharov_start, $totalstarttime_temp;
                if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
                    $query_time = microtime(true) - $start_time;     //12    (12-2-3=7)
                    ...
                    insert into logwawi set
                      ...
                      sql_text='" . addslashes($query . ":" . print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), 1)) . "'
                ");
                }
    ...
    public function executeUpdate($query, $parameters = [], $types = [])
    {
        ...
        global $scharov_start, $totalstarttime_temp;
        if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
            if (substr(trim($query),0,19)<>'insert into logwawi') {
                $query_time = microtime(true) - $start_time;     //12    (12-2-3=7)
                $vordem_zeit = microtime(true) - $totalstarttime_temp;
                $totalstarttime_temp = microtime(true);
