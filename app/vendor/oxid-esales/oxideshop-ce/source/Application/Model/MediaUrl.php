...
class MediaUrl extends \OxidEsales\Eshop\Core\Model\MultiLanguageModel
{
    ...
    protected function _getYoutubeHtml()
    {
        ...
        if (strpos($sUrl, 'youtube.com')) {
            $sYoutubeUrl = str_replace("www.youtube.com/watch?v=", "", $sUrl);
            $sYoutubeUrl = preg_replace('/&amp;/', '?', $sYoutubeUrl, 1);
        }
        if (strpos($sUrl, 'youtu.be')) {
            $sYoutubeUrl = str_replace("youtu.be/", "", $sUrl);
        }


        $sYoutubeUrl = str_replace("https://", "", $sYoutubeUrl);
        $sYoutubeUrl = str_replace("http://", "", $sYoutubeUrl);

        $sYoutubeTemplate = '%s<br><div id="video" class="block-videocontainer"><div class="videocontainer-wrapper"><div class="videocontainer"><div class="youtube-iframe" id="%s" style="width: 500px; height: 281px;"></div></div></div></div>';
