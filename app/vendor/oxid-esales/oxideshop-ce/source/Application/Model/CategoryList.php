...
class CategoryList extends \OxidEsales\Eshop\Core\Model\ListModel
{
    ...
    public function load()
    {
        ...
        $stop_db = microtime(true) - $start_db;
        global $scharov_start, $totalstarttime_temp;
        if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
            $vordem_zeit = microtime(true) - $totalstarttime_temp;
            $totalstarttime_temp = microtime(true);

            \oxDb::getDb()->executeUpdate("
                    insert into logwawi set
                      processid = '" . getmypid() . "',
                      date_create = '" . time() . "',
                      ip = '" . ip2long($_SERVER["REMOTE_ADDR"]) . "',
                      oxcl = '" . addslashes($_REQUEST["cl"]) . "',
                      oxfnc = '" . addslashes($_REQUEST["fnc"]) . "',
                      oxuserid='" . addslashes($_SESSION["auth"]) . "',
                      suri='" . addslashes($_SERVER["REQUEST_URI"]) . "',
                      logtype='mollie',
                      vordem_zeit='" . $vordem_zeit . "',
                      sql_text='categorie assignArray start'
                ");
        }

        $start_data = microtime(true);
        $this->assignArray($aData);
        $stop_data = microtime(true) - $start_data;
        ...
        if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
            ...
            \oxDb::getDb()->executeUpdate("
                    insert into logwawi set
                      ...
                      logtype='mollie',
                      vordem_zeit='" . $vordem_zeit . "',
                      sql_text='categorie assignArray end'
                ");
        }
        ...
        if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
            ...
            \oxDb::getDb()->executeUpdate("
                    insert into logwawi set
                      ...
                      logtype='mollie',
                      vordem_zeit='" . $vordem_zeit . "',
                      sql_text='category _ppBuildTree start'
                ");
        }
        $aTree = [];
        ...
        foreach ($this->_aArray as $oCat) {
            ...
            if ($sParentId != 'oxrootid') {
                if (isset($this->_aArray[$sParentId])) {
                    ...
                    ++$i;
                    $stop_b = microtime(true) - $start_b;
                }
            } else {
                $aTree[$oCat->getId()] = $oCat;
            }
        }
        $stop_a = microtime(true) - $start_a;
        global $scharov_start, $totalstarttime_temp;
        if ($scharov_start==$_SERVER["REMOTE_ADDR"]) {
            $vordem_zeit = microtime(true) - $totalstarttime_temp;
            $totalstarttime_temp = microtime(true);
            ...
            \oxDb::getDb()->executeUpdate("
                    insert into logwawi set
                      ...
                      logtype='mollie',
                      vordem_zeit='" . $vordem_zeit . "',
                      sql_text='category _ppBuildTree end'
                ");
        }
        $this->assign($aTree);
